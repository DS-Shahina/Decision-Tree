...
X = data.drop(['Taxable.Income_Good'], axis=1)
y = data['Taxable.Income_Good']
...
np.mean(preds == y_test) # Test Data Accuracy  - 75%
...
np.mean(preds == y_test) # Test Data Accuracy  - 79%

# Prediction on Train Data
preds = forest_new.predict(X_train)
pd.crosstab(preds, y_train, rownames = ['Predictions'], colnames = ['Actual'])

np.mean(preds == y_train) # Train Data Accuracy # 79% accuracy 

# it improves model accuracy with 79% of accuracy
